...
class Method_MLP(method, nn.Module):
    ...
    def forward(self, x):
        ...
        y_pred = self.activation_func_4(self.fc_layer_4(h_3))
    ...
    def train(self, X, y):
        ...
        metrics_evaluator = Evaluate_Metrics('training evaluator', '')
        ...
        for epoch in range(self.max_epoch): # you can do an early stop if self.max_epoch is too much...
            ...
            if epoch%50 == 0:
                metrics_evaluator.data = {'true_y': y_true, 'pred_y': y_pred.max(1)[1]}
                self.loss.append(train_loss.item())
                print('Epoch:', epoch, 'Accuracy:', metrics_evaluator.evaluateAccuracy(),
                      'Precision:', metrics_evaluator.evaluatePrecision('macro'),
                      'Recall:', metrics_evaluator.evaluateRecall('macro'),
                      'F1 Score:', metrics_evaluator.evaluateF1('macro'),
                      'Loss:', self.loss[-1])
